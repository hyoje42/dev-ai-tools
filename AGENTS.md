...
- **Cross-submodule commit gate**: before proposing or performing a commit when only one submodule has been changed, always check whether the other submodule has a corresponding rule, skill, or configuration that should be updated too. Use `check-sync-status` for paired skills and inspect the relevant counterparts before deciding whether to port, convert, or intentionally leave the change tool-specific, and record the conversion rationale in [docs/skill-sync-status.md](./docs/skill-sync-status.md). Do not proceed with the commit until this check is complete. This is a parent-repo orchestration rule; do not copy it into the submodules' own instruction files.
...
- **Language**: agent-loaded instruction files (AGENTS.md — including Codex's `home/AGENTS.md` global instructions — Claude's `home/rules/`, and skills) are written in **English**; human-facing docs (README and notes/guides like `skill-authoring.md` and `docs/`) are written in **Korean**. A language-specific output style body whose grammar, morphology, or contrastive examples would lose precision in translation may remain in the target language only when its submodule README records the reason and manual update policy. Sync/utility scripts follow the same split — **user-facing output (`echo`/errors) in Korean**; code comments may stay in English.
