...
두 도구의 설정을 git submodule로 묶어 한 번에 clone·버전 관리하고, 각 도구 홈 디렉터리(`~/.claude/`, `~/.codex/`)와 동기화한다.
...
두 도구 모두 같은 패턴을 따른다(도구별 구체 명령은 각 repo의 README 참고).

1. submodule의 `home/` 하위(rules·skills·전역 설정)를 수정한다.
2. `*-diff-with-home`으로 `~/.<tool>/`과의 차이를 확인해 공유한다.
